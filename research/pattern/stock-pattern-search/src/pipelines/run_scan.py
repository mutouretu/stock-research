...
def _load_reviewer(config: Dict[str, Any]) -> NewHighReviewConfig | None:
    reviewer = config.get("reviewer")
    if reviewer in (None, "", "none", False):
        return None
    if isinstance(reviewer, str):
        if reviewer != "new_high_near_3y":
            raise ValueError(f"unknown reviewer: {reviewer}")
        return NewHighReviewConfig()
    if not isinstance(reviewer, dict):
        raise ValueError("reviewer must be a mapping, string, or null")

    if not bool(reviewer.get("enabled", True)):
        return None
    name = str(reviewer.get("name", "new_high_near_3y"))
    if name != "new_high_near_3y":
        raise ValueError(f"unknown reviewer: {name}")
    return NewHighReviewConfig.from_dict(reviewer)


def main(config_path: str = "configs/common/infer.yaml") -> pd.DataFrame:
    config = _load_yaml(config_path)

    raw_daily_dir = Path(config.get("raw_daily_dir", "data/raw/daily"))
    model_dir = Path(config.get("model_dir", "outputs/models/common/baseline_lr"))
    output_path = Path(config.get("output_path", "outputs/predictions/common/scan_predictions.csv"))
    window_size = int(config.get("window_size", 120))
    min_history = int(config.get("min_history", 160))
    configured_asof_date = config.get("asof_date")
    require_exact_asof_date = bool(config.get("require_exact_asof_date", False))
    reviewer_config = _load_reviewer(config)
    ...
    for path in parquet_files:
        ...
        try:
            ...
            asof_date = pd.to_datetime(configured_asof_date, errors="coerce") if configured_asof_date else pd.to_datetime(
                daily_df["trade_date"].iloc[-1], errors="coerce"
            )
            if pd.isna(asof_date):
                print(f"[skip] {ts_code}: invalid latest trade_date")
                continue
            latest_trade_date = pd.to_datetime(daily_df["trade_date"].iloc[-1], errors="coerce")
            if require_exact_asof_date and latest_trade_date.normalize() != asof_date.normalize():
                print(f"[skip] {ts_code}: latest trade_date {latest_trade_date:%Y-%m-%d} != asof_date {asof_date:%Y-%m-%d}")
                continue
            ...
            if reviewer_config is not None:
                review_fields = evaluate_new_high_review(daily_df, asof_date=asof_date, config=reviewer_config)
                if not bool(review_fields.get("new_high_review_pass", False)):
                    continue
    ...
    pred_df["model_score"] = scores
    if "new_high_score_factor" in pred_df.columns:
        factor = pd.to_numeric(pred_df["new_high_score_factor"], errors="coerce").fillna(1.0)
        pred_df["score"] = pred_df["model_score"] * factor
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="configs/common/infer.yaml", help="Path to infer config yaml")
