...
def _prepare_baseline(
    ...
    *,
    ...
    passthrough_cols = [col for col in passthrough_candidates if col in df.columns]
    df = df[KEY_COLUMNS + passthrough_cols + ["baseline_rank", score_col]].copy()
...
def build_review_candidates(config_path: str | Path) -> pd.DataFrame:
    ...
    output_path = _resolve_path(project_root, config.get("output_path", "outputs/predictions/common/review_candidates.csv"))
    ...
    front_cols = KEY_COLUMNS + ["review_rank"]
    if "anchor_date" in result.columns:
        front_cols.append("anchor_date")
    front_cols += ["baseline_name", "baseline_rank", "baseline_score"]
